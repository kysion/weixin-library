...
func (s *sUserAuth) UserLogin(ctx context.Context, info g.Map) (string, error) {
	...
	if openID != "" {
		if res.Unionid != "" {
			weiXinConsumer, err = weixin_service.Consumer().GetConsumerByOpenId(ctx, openID, res.Unionid)
		} else {
			weiXinConsumer, err = weixin_service.Consumer().GetConsumerByOpenId(ctx, openID)
		}
	...
	err = dao.WeixinConsumerConfig.Transaction(ctx, func(ctx context.Context, tx gdb.TX) error {
		...
		if weiXinConsumer == nil { // 创建
			...
			wConsumerData.UnionId = res.Unionid
			wConsumerData.OpenId = res.Openid
			wConsumerData.SessionKey = res.SessionKey
...
// 小程序获取用户信息
func miniAppGetUserInfo(sessionKey, encryptedData, iv string, appId string) (*wxUserInfo, error) {
	url := fmt.Sprintf("https://api.weixin.qq.com/wxa/getuserinfo?access_token=%s&openid=%s&lang=zh_CN", sessionKey, appId)
...
func (s *sUserAuth) GetTinyAppUserInfo(ctx context.Context, sessionKey, encryptedData, iv, appId string) (*weixin_model.UserInfoRes, error) {
	// 假设这里已经获取到了用户的 sessionKey 和 encryptedData
	//sessionKey := "<your_session_key>"
	//encryptedData := "<your_encrypted_data>"
	//iv := "<your_iv>"

	userInfo, err := miniAppGetUserInfo(sessionKey, encryptedData, iv, appId)
	if err != nil {
